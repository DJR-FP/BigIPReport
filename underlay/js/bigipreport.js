...
window.addEventListener('load', () => bigipreport_awaiter(void 0, void 0, void 0, function* () {
    ...
    <div class="footer">
      ...
      ${Math.round(siteData.preferences.executionTime).toString()} minutes to finish.
    </div>
  `);
    /* ************************************************************************************************************
                ...
                $.ajax({
                    ...
                    success(data) {
                        if (data.success) {
                            decreaseAjaxQueue(url);
                            Object.keys(data.memberstatuses).forEach(member => {
                                const statusSpan = $(`table#poolTable details[data-loadbalancer="${loadbalancerName}"][data-name="${poolName}"] ` +
...
function setupPoolTable() {
    ...
    $('div#poolTable_filter.dataTables_filter input').on('keyup input', () => {
        updateLocationHash();
    });
    // highlight matches
